...
//     status: Yup.string().required('Status is required'),
//     receivedAt: Yup.date().required('Received date is required'),
//     estimatedDeliveryAt: Yup.date().required('estimatedDelivery date is required'),
//     deliveredAt: Yup.date().nullable(),
//     trackingNumber: Yup.string().required('Tracking number is required'),
//   });
  
//   const onSubmit = async (values, { setSubmitting }) => {
//     console.log("Updated Parcel Form Data",values);
//     try {
//         const parcelData = {
//         ...values,
//         receivedAt: new Date(values.receivedAt).toISOString(),
...
//         // Remove undefined/null values
//         const cleanUpdatedParcelData = Object.fromEntries(
//           Object.entries(parcelData).filter(([_, v]) => v !== null && v !== undefined)
//         );

//       await updateParcel(id, cleanUpdatedParcelData);
//       navigate('/');
//       console.log("Updated Parcel Form Data",values);
//     } catch (error) {
//       console.error('Error updating parcel:', error);
...
//           recipientEmail: parcel.recipientEmail || '',
//           status: parcel.status || 'RECEIVED',
//           receivedAt: parcel.receivedAt ? new Date(parcel.receivedAt).toISOString().slice(0, 16) : new Date().toISOString().slice(0, 16),
//           estimatedDeliveryAt: parcel.estimatedDeliveryAt? new Date(parcel.estimatedDeliveryAt).toISOString().slice(0, 16) : '',
//           deliveredAt: parcel.deliveredAt ? new Date(parcel.deliveredAt).toISOString().slice(0, 16) : '',
//           trackingNumber: parcel.trackingNumber || '',
//         }}
//         validationSchema={validationSchema}
//         onSubmit={onSubmit}
...
//              <div>
//                 <label className="block text-sm font-medium text-gray-700">Estimated Delivery At</label>
//                 <Field
//                   type="datetime-local"
//                   name="estimatedDeliveryAt"
//                   className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
